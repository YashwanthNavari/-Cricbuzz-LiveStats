...
if not DATABASE_URL:
    raise ValueError("DATABASE_URL environment variable is not set. Please set it in .env or Streamlit Secrets.")
        ...
        if "matches" in inspector.get_table_names():
            ...
            if "match_overs_limit" not in columns:
                with engine.begin() as conn:
                    conn.execute(text("ALTER TABLE matches ADD COLUMN match_overs_limit INTEGER"))
            if "player_of_the_match_id" not in columns:
                with engine.begin() as conn:
                    conn.execute(text("ALTER TABLE matches ADD COLUMN player_of_the_match_id INTEGER REFERENCES players(id)"))
        ...
        if "innings" in inspector.get_table_names():
            ...
            for col in ["extras", "wides", "no_balls", "byes", "leg_byes"]:
                if col not in columns:
                    with engine.begin() as conn:
                        conn.execute(text(f"ALTER TABLE innings ADD COLUMN {col} INTEGER DEFAULT 0"))
