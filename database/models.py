...
class Match(Base):
    ...
    toss_winner_id: Mapped[Optional[int]] = mapped_column(
    ...
    match_start_time: Mapped[Optional[datetime]] = mapped_column(
    ...
    player_of_the_match_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("players.id", ondelete="SET NULL")
    )

    series: Mapped[Optional["Series"]] = relationship(
        "Series", back_populates="matches"
    )
    ...
    toss_winner: Mapped[Optional["Team"]] = relationship(
        "Team", foreign_keys=[toss_winner_id]
    )
    winner: Mapped[Optional["Team"]] = relationship("Team", foreign_keys=[winner_id])
    player_of_the_match: Mapped[Optional["Player"]] = relationship(
        "Player", foreign_keys=[player_of_the_match_id]
    )
...
class Innings(Base):
    ...
    __table_args__ = (
        UniqueConstraint("match_id", "innings_num", name="unique_match_innings"),
    )
    ...
    batting_scores: Mapped[List["BattingScore"]] = relationship(
        "BattingScore", back_populates="innings", cascade="all, delete-orphan"
    )
    bowling_scores: Mapped[List["BowlingScore"]] = relationship(
        "BowlingScore", back_populates="innings", cascade="all, delete-orphan"
    )
    fielding_records: Mapped[List["FieldingRecord"]] = relationship(
        "FieldingRecord", back_populates="innings", cascade="all, delete-orphan"
    )
    partnerships: Mapped[List["Partnership"]] = relationship(
        "Partnership", back_populates="innings", cascade="all, delete-orphan"
    )
...
class BattingScore(Base):
    ...
    __table_args__ = (
        UniqueConstraint("innings_id", "player_id", name="unique_innings_batsman"),
    )
    ...
    innings: Mapped["Innings"] = relationship(
        "Innings", back_populates="batting_scores"
    )
    player: Mapped["Player"] = relationship("Player", foreign_keys=[player_id])
    bowler: Mapped[Optional["Player"]] = relationship(
        "Player", foreign_keys=[bowler_id]
    )
    fielder: Mapped[Optional["Player"]] = relationship(
        "Player", foreign_keys=[fielder_id]
    )
...
class BowlingScore(Base):
    ...
    __table_args__ = (
        UniqueConstraint("innings_id", "player_id", name="unique_innings_bowler"),
    )
    ...
    innings: Mapped["Innings"] = relationship(
        "Innings", back_populates="bowling_scores"
    )
...
class FieldingRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint("innings_id", "player_id", name="unique_innings_fielder"),
    )
    ...
    innings: Mapped["Innings"] = relationship(
        "Innings", back_populates="fielding_records"
    )
    player: Mapped["Player"] = relationship("Player")
...
class Partnership(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "innings_id", "batsman1_id", "batsman2_id", name="unique_partnership"
        ),
    )
    ...
    batsman1_id: Mapped[int] = mapped_column(
        ForeignKey("players.id", ondelete="CASCADE")
    )
    batsman2_id: Mapped[int] = mapped_column(
        ForeignKey("players.id", ondelete="CASCADE")
    )
    runs: Mapped[int] = mapped_column(Integer, default=0)
    balls: Mapped[int] = mapped_column(Integer, default=0)
