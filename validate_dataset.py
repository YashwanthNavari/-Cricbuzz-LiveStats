...
from database.models import Player, Team, Venue, Match, Innings, BattingScore, BowlingScore
...
class DatasetValidator:
    ...
    def run_validation(self) -> Optional[Dict[str, Any]]:
        ...
        with get_db() as session:
            ...
            dup_players = session.execute(text(
            ...
            dup_teams = session.execute(text(
            ...
            dup_venues = session.execute(text(
            ...
            tables = ["series", "venues", "teams", "players", "matches", "innings", "batting_scores", "bowling_scores"]
            for table in tables:
                null_pk = session.execute(text(f"SELECT COUNT(*) FROM {table} WHERE id IS NULL")).scalar()
                if null_pk > 0:
            ...
            null_matches = session.execute(text(
            ...
            broken_venues = session.execute(text(
            ...
            broken_players = session.execute(text(
            ...
            bad_overs_inn = session.execute(text(
    ...
    def auto_fix_issues(self) -> int:
        ...
        try:
            with get_db() as session:
                ...
                bad_overs = session.execute(text(
                ...
                for row in bad_overs:
                    ...
                    if frac >= 6:
                        ...
                        fixed_count += 1
                        logger.info(f"Fixed invalid over count for Innings ID {inn_id}: {overs} -> {new_overs}")
    ...
    def generate_reports(self, results: Dict[str, Any]) -> None:
        ...
        with open(md_file, "w", encoding="utf-8") as f:
            ...
            f.write(f"| Missing Key Values (NULL fields) | {results['null_values_key_fields']} |\n")
            f.write(f"| Broken Foreign Keys (Orphans) | {results['broken_foreign_keys']} |\n")
            f.write(f"| Incorrect Overs Formats | {results['incorrect_overs_format']} |\n\n")
            ...
            if not results["details"]:
                ...
                for det in results["details"]:
                    f.write(f"| {det['table']} | {det['type']} | {det['description']} | {det['severity']} |\n")
        
        logger.info(f"Validation Markdown report saved: processed_data/reports/validation_report_{self.timestamp}.md")
        ...
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            ...
            for det in results["details"]:
                writer.writerow([det["table"], det["type"], det["description"], det["severity"]])
        
        logger.info(f"Validation CSV report saved: processed_data/reports/validation_errors_{self.timestamp}.csv")
