...
def render_logs_page():
    st.markdown("<h2 style='color: #1e90ff;'>📜 Log Viewer</h2>", unsafe_allow_html=True)
    st.write("Monitor system logs for API transactions, database transactions, parsing operations, and validation runs.")
    ...
    if not log_file_path.exists():
        st.info("No log file found. Once you run the API explorer or dataset builder, pipeline log streams will be written here.")
        return
    ...
    with col_level:
        level_filter = st.selectbox("Log Level Filter", ["ALL", "INFO", "WARNING", "ERROR", "CRITICAL"])
