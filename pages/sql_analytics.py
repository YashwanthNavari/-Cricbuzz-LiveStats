...
def get_filter_options():
    ...
    try:
        with get_db() as session:
            # Series
            res_series = session.execute(text("SELECT id, name FROM series ORDER BY name")).fetchall()
            for r in res_series:
                series_list.append((r[0], f"{r[1]} (ID: {r[0]})"))
            
            # Teams
            res_teams = session.execute(text("SELECT id, name FROM teams ORDER BY name")).fetchall()
            for r in res_teams:
                teams_list.append((r[0], f"{r[1]} (ID: {r[0]})"))
                
            # Players
            res_players = session.execute(text("SELECT id, name FROM players ORDER BY name")).fetchall()
...
def render_sql_analytics():
    st.markdown("<h2 style='color: #1e90ff;'>📊 SQL Analytics Engine</h2>", unsafe_allow_html=True)
    st.write("Execute and visualize the 25 pre-defined SQL queries using parameters selected directly from your database.")
    ...
    query_categories = {
        ...
        "🥎 Bowling, Fielding & Extras": ["Q3", "Q5", "Q8", "Q9", "Q13", "Q14", "Q16", "Q20", "Q21", "Q23"]
    }
    
    selected_cat = st.selectbox("Filter Questions by Category", list(query_categories.keys()))
        ...
        with st.expander(f"🔍 {key}: {q_info['title']}"):
            ...
            if q_info["params"]:
                ...
                for idx, (p_name, p_type) in enumerate(q_info["params"].items()):
                    with cols[idx]:
                        if p_type == "series":
                            selected_label = st.selectbox(f"Filter by Series ({p_name})", list(series_map.keys()), key=f"{key}_{p_name}")
                            bind_params[p_name] = series_map[selected_label]
                        elif p_type == "team":
                            selected_label = st.selectbox(f"Filter by Team ({p_name})", list(team_map.keys()), key=f"{key}_{p_name}", index=min(idx, len(team_map)-1))
                            bind_params[p_name] = team_map[selected_label]
                        elif p_type == "player":
                            selected_label = st.selectbox(f"Filter by Player ({p_name})", list(player_map.keys()), key=f"{key}_{p_name}")
            ...
            if run_btn:
                try:
                    ...
                    with get_db() as session:
                        ...
                        df_res = pd.read_sql_query(sql_stmt, session.bind, params=bind_params)
                    ...
                    if df_res.empty:
                        ...
                        if q_info.get("chart_type") and q_info["chart_type"] != "table":
                            ...
                            if isinstance(kw, dict):
                                try:
                                    if c_type == "bar":
                                        fig = px.bar(df_res, **kw, template="plotly_dark")
                                        st.plotly_chart(fig, use_container_width=True)
                                    elif c_type == "pie":
                                        fig = px.pie(df_res, **kw, template="plotly_dark")
                                        st.plotly_chart(fig, use_container_width=True)
                                    elif c_type == "scatter":
                                        fig = px.scatter(df_res, **kw, template="plotly_dark")
                                        st.plotly_chart(fig, use_container_width=True)
                                    elif c_type == "line":
                                        fig = px.line(df_res, **kw, template="plotly_dark")
                                        st.plotly_chart(fig, use_container_width=True)
                                except Exception as chart_err:
                                    st.warning(f"Could not render Plotly chart: {chart_err}")
