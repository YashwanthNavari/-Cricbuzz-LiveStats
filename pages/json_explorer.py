...
def render_json_explorer():
    st.markdown(
        "<h2 style='color: #1e90ff;'>📂 JSON Explorer</h2>", unsafe_allow_html=True
    )
    st.write(
        "Browse and analyze raw JSON responses saved inside the local `raw_data` cache folder."
    )
    ...
    if not json_files:
        st.info(
            "No raw JSON files found in `raw_data/` directory. Fetch some data first via API Explorer or Dataset Builder!"
        )
        return
    ...
    selected_rel_path = st.selectbox(
        "Select JSON Cache File", list(file_options.keys())
    )
    ...
    time_formatted = datetime.datetime.fromtimestamp(time_str).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
        ...
        def search_json(d, path=""):
            if isinstance(d, dict):
                for k, v in d.items():
                    curr_path = f"{path}.{k}" if path else k
                    if (
                        search_query.lower() in str(k).lower()
                        or search_query.lower() in str(v).lower()
                    ):
