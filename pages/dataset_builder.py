...
def render_dataset_builder():
    st.markdown("<h2 style='color: #1e90ff;'>⚙️ Dataset Builder</h2>", unsafe_allow_html=True)
    st.write("Populate your local SQL database by running the Cricbuzz RapidAPI ETL (Extract, Transform, Load) ingestion pipeline.")
    ...
    with col_stats:
        ...
        st.info("🔄 Resume capability: Active. Match details already residing in the database will be automatically skipped to conserve your RapidAPI quota.")
    ...
    if run_btn:
        ...
        try:
            # Stage 1
            status_text.text("Connecting to RapidAPI Host and pulling match listings...")
            progress_bar.progress(20)
            
            pipeline = IngestionPipeline()
            
            # Stage 2
            status_text.text("Downloading raw scorecard payloads and writing local cache...")
            progress_bar.progress(50)
            
            # Stage 3
            status_text.text("Parsing scorecards and writing normalized tables into SQL...")
            ...
            sc2.metric("New Scorecards normalise", results.get("scorecards_ingested", 0))
