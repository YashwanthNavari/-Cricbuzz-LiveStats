...
def render_crud_operations():
    st.markdown("<h2 style='color: #1e90ff;'>🛠️ CRUD Operations</h2>", unsafe_allow_html=True)
    st.write("Perform Create, Read, Update, and Delete operations on core cricket entities and review the raw SQL statements triggered.")
    ...
    with tab_player:
        ...
        action = st.radio("Choose Action", ["Create Player", "Read/Update/Delete Player"], key="player_action")
        ...
        if action == "Create Player":
            with st.form("create_player_form"):
                ...
                if submitted:
                    if not p_name:
                        ...
                        params = {"id": p_id, "name": p_name, "role": p_role, "bat": p_bat, "bowl": p_bowl}
                        
                        st.markdown("**SQL Executed:**")
                        st.code(sql_stmt.replace(":id", str(p_id)).replace(":name", f"'{p_name}'").replace(":role", f"'{p_role}'").replace(":bat", f"'{p_bat}'").replace(":bowl", f"'{p_bowl}'"), language="sql")
            ...
            try:
                with get_db() as session:
                    res = session.execute(text("SELECT id, name FROM players ORDER BY name")).fetchall()
            ...
            if not players_list:
                ...
                sel_label = st.selectbox("Select Player Record", list(player_map.keys()))
                sel_id = player_map[sel_label]
                
                # Fetch details
                try:
                    with get_db() as session:
                        player_row = session.execute(text("SELECT * FROM players WHERE id = :id"), {"id": sel_id}).fetchone()
                except Exception as e:
                ...
                if player_row:
                    with st.form("edit_player_form"):
                        ...
                        edit_bat = st.text_input("Batting Style", value=player_row[3] or "")
                        edit_bowl = st.text_input("Bowling Style", value=player_row[4] or "")
                        ...
                        if update_submit:
                            ...
                            params = {"id": sel_id, "name": edit_name, "role": edit_role, "bat": edit_bat, "bowl": edit_bowl}
                            
                            st.markdown("**SQL Executed:**")
                            st.code(sql_stmt.replace(":id", str(sel_id)).replace(":name", f"'{edit_name}'").replace(":role", f"'{edit_role}'").replace(":bat", f"'{edit_bat}'").replace(":bowl", f"'{edit_bowl}'"), language="sql")
                        ...
                        if delete_submit:
                            ...
                            try:
                                ...
                                st.error(f"Failed to delete player (Cascade rules apply): {e}")

    # ------------------ TEAMS TAB ------------------
    with tab_team:
        st.subheader("Team Record Management")
        t_action = st.radio("Choose Action", ["Create Team", "Read/Update/Delete Team"], key="team_action")
        ...
        if t_action == "Create Team":
            with st.form("create_team_form"):
                ...
                if submitted:
                    if not t_name:
                        ...
                        st.markdown("**SQL Executed:**")
                        st.code(sql_stmt.replace(":id", str(t_id)).replace(":name", f"'{t_name}'").replace(":sname", f"'{t_sname}'"), language="sql")
            ...
            try:
                with get_db() as session:
                    res = session.execute(text("SELECT id, name FROM teams ORDER BY name")).fetchall()
            ...
            if not teams_list:
                ...
                try:
                    with get_db() as session:
                        team_row = session.execute(text("SELECT * FROM teams WHERE id = :id"), {"id": sel_id}).fetchone()
                except Exception as e:
                    st.error(f"Failed to fetch details: {e}")
                    return
                ...
                if team_row:
                    with st.form("edit_team_form"):
                        ...
                        edit_sname = st.text_input("Short Name", value=team_row[2] or "")
                        ...
                        if update_submit:
                            ...
                            params = {"id": sel_id, "name": edit_name, "sname": edit_sname}
                            
                            st.markdown("**SQL Executed:**")
                            st.code(sql_stmt.replace(":id", str(sel_id)).replace(":name", f"'{edit_name}'").replace(":sname", f"'{edit_sname}'"), language="sql")
                        ...
                        if delete_submit:
                            ...
                            try:
                                ...
                                st.error(f"Failed to delete team (Cascade rules apply): {e}")

    # ------------------ MATCHES TAB ------------------
    with tab_match:
        st.subheader("Match Record Management")
        st.info("Match records reference Series, Venues, and Teams. Use the database viewer or cascade operations as required. Creating/Deleting matches is supported.")
        m_action = st.radio("Choose Action", ["Create Match", "Delete Match"], key="match_action")
        ...
        if m_action == "Create Match":
            ...
            try:
                with get_db() as session:
                    res_s = session.execute(text("SELECT id, name FROM series")).fetchall()
                    series_opts = [(r[0], r[1]) for r in res_s]
                    res_v = session.execute(text("SELECT id, name FROM venues")).fetchall()
                    venue_opts = [(r[0], r[1]) for r in res_v]
                    res_t = session.execute(text("SELECT id, name FROM teams")).fetchall()
            ...
            if not team_opts:
                ...
                with st.form("create_match_form"):
                    ...
                    m_series = st.selectbox("Select Series", [None] + [s[0] for s in series_opts], format_func=lambda x: dict(series_opts).get(x, "None"))
                    m_venue = st.selectbox("Select Venue", [None] + [v[0] for v in venue_opts], format_func=lambda x: dict(venue_opts).get(x, "None"))
                    
                    m_t1 = st.selectbox("Team 1", [t[0] for t in team_opts], format_func=lambda x: dict(team_opts).get(x))
                    m_t2 = st.selectbox("Team 2", [t[0] for t in team_opts], format_func=lambda x: dict(team_opts).get(x), index=min(1, len(team_opts)-1))
                    ...
                    if submitted:
                        ...
                        """
                        params = {
                            "id": m_id, "series_id": m_series, "venue_id": m_venue, 
                            "desc": m_desc, "format": m_format, "status": m_status, 
                            "team1": m_t1, "team2": m_t2, "overs_lim": overs_lim
                        }
                        
                        st.markdown("**SQL Executed:**")
                        st.code(sql_stmt.replace(":id", str(m_id)).replace(":series_id", str(m_series)).replace(":venue_id", str(m_venue)).replace(":desc", f"'{m_desc}'").replace(":format", f"'{m_format}'").replace(":status", f"'{m_status}'").replace(":team1", str(m_t1)).replace(":team2", str(m_t2)).replace(":overs_lim", str(overs_lim)), language="sql")
            ...
            try:
                with get_db() as session:
                    res = session.execute(text("SELECT id, match_desc FROM matches")).fetchall()
                ...
                if delete_btn:
                    ...
                    try:
                        with get_db() as session:
                            session.execute(text(sql_stmt), {"id": sel_id})
                            session.commit()
                        st.success("Match deleted successfully (innings and scorecard details cascadingly removed).")
                        st.write("Affected Rows: 1")
                    except Exception as e:
    ...
    with tab_series:
        ...
        s_action = st.radio("Choose Action", ["Create Series", "Delete Series"], key="series_action")
        ...
        if s_action == "Create Series":
            with st.form("create_series_form"):
                ...
                if submitted:
                    if not s_name:
                        ...
                        st.markdown("**SQL Executed:**")
                        st.code(sql_stmt.replace(":id", str(s_id)).replace(":name", f"'{s_name}'").replace(":type", f"'{s_type}'"), language="sql")
            ...
            try:
                with get_db() as session:
                    res = session.execute(text("SELECT id, name FROM series")).fetchall()
    ...
    with tab_venue:
        ...
        v_action = st.radio("Choose Action", ["Create Venue", "Delete Venue"], key="venue_action")
        ...
        if v_action == "Create Venue":
            with st.form("create_venue_form"):
                ...
                if submitted:
                    if not v_name:
                        ...
                        params = {"id": v_id, "name": v_name, "city": v_city, "country": v_country, "capacity": v_capacity}
                        
                        st.markdown("**SQL Executed:**")
                        st.code(sql_stmt.replace(":id", str(v_id)).replace(":name", f"'{v_name}'").replace(":city", f"'{v_city}'").replace(":country", f"'{v_country}'").replace(":capacity", str(v_capacity)), language="sql")
            ...
            try:
                with get_db() as session:
                    res = session.execute(text("SELECT id, name FROM venues")).fetchall()
