...
def render_database_viewer():
    st.markdown(
        "<h2 style='color: #1e90ff;'>🗄️ Database Viewer</h2>", unsafe_allow_html=True
    )
    st.write(
        "Browse table schemas, primary keys, foreign keys, indexes, and preview normalized data records with pagination."
    )
    ...
    if not tables:
        st.warning(
            "No tables found in the database. Execute the pipeline or initialize the schema first."
        )
        return
