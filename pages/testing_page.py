...
def run_db_conn_test():
    ...
    try:
        ...
        return True, f"Connected successfully in {(time.time() - start)*1000:.1f}ms", None
...
def run_tables_check_test():
    ...
    expected = {"series", "venues", "teams", "players", "matches", "innings", "batting_scores", "bowling_scores", "fielding_records", "partnerships"}
    try:
...
def run_api_conn_test():
    ...
    start = time.time()
    try:
        res = requests.get(f"https://{api_host}/matches/v1/recent", headers=headers, timeout=10)
        if res.status_code in (200, 401, 403, 429):
            return True, f"API Host reached. Status code: {res.status_code}. Time: {(time.time() - start)*1000:.1f}ms", None
...
def run_crud_test():
    ...
    try:
        with get_db() as session:
            # 1. Insert test team
            session.execute(text("INSERT INTO teams (id, name, short_name) VALUES (9999, 'Test Team', 'TST')"))
            # 2. Update
            session.execute(text("UPDATE teams SET name = 'Test Team Updated' WHERE id = 9999"))
            # 3. Read & Verify
            res = session.execute(text("SELECT name FROM teams WHERE id = 9999")).fetchone()
            ...
            session.commit()
        return True, "CRUD operations test passed (Insert, Update, Read, Delete verified).", None
    except Exception as e:
        return False, "CRUD operations test failed.", str(e)

def run_pipeline_unit_tests():
    """Runs tests/test_pipeline.py programmatically."""
    from tests.test_pipeline import TestCricbuzzClient, TestValidator, TestTransformer, TestIngestionPipeline
...
def render_testing_page():
    st.markdown("<h2 style='color: #1e90ff;'>🧪 Testing & QA Center</h2>", unsafe_allow_html=True)
    st.write("Run automated system integration tests and standard python unit test suites programmatically.")
    ...
    if run_tests_btn:
        ...
        test_cases = [
            ...
            ("🧬 Ingestion Pipeline Unit Tests (test_pipeline.py)", run_pipeline_unit_tests)
        ]
        ...
        for name, test_func in test_cases:
            ...
            try:
                ...
                pass_status, desc, details = False, "Test crashed with unhandled exception.", str(e)
