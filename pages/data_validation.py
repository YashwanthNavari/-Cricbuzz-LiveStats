...
def render_data_validation():
    st.markdown(
        "<h2 style='color: #1e90ff;'>🛡️ Data Validation & Quality</h2>",
        unsafe_allow_html=True,
    )
    st.write(
        "Ensure data consistency. Run diagnostics for duplicates, broken links, missing attributes, or bad formatting."
    )
    ...
    with col_ctrl:
        st.write(
            "Click below to run a validation check against all relational tables in the database:"
        )
        check_clicked = st.button("Run Diagnostic Check 🔍", use_container_width=True)

    with col_act:
        st.write(
            "Run the Auto-Fix engine to repair formatting anomalies (e.g. over counts):"
        )
    ...
    if fix_clicked:
        ...
        try:
            ...
            if repaired_count > 0:
                st.success(
                    f"Auto-fix completed successfully! Repaired {repaired_count} over format records in the database."
                )
            else:
    ...
    if check_clicked or fix_clicked:
        ...
        card1.metric(
            "Duplicates (Players/Teams/Venues)",
            results["duplicate_players"]
            + results["duplicate_teams"]
            + results["duplicate_venues"],
        )
        ...
        if total_issues == 0:
            ...
            st.error(
                f"❌ CRITICAL ERROR: Database contains {total_issues} issues, including orphaned foreign keys or missing PKs."
            )
        else:
            st.warning(
                f"⚠️ WARNING: Database contains {total_issues} issues. Check details below."
            )
