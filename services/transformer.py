...
def transform_match(data: Dict[str, Any], series_id: Optional[int] = None) -> Match:
    ...
    start_time = parse_date(match_info.get("startDate") or match_info.get("matchStartTimestamp"))
    ...
    for raw_inn in raw_scorecards:
        ...
        for raw_bat in batsmen_list:
            ...
            if out_desc:
                ...
                if desc.startswith("c ") or desc.startswith("st "):
                    ...
                    for p in match_players:
                        if p.name.lower() in fielder_part or fielder_part in p.name.lower():
                            if p.id != bowler_id:
                                fielder_id = p.id
                                break
        ...
        if isinstance(part_data, dict):
            ...
            for raw_part in part_list:
                ...
                fours = int(raw_part.get("bat1fours", 0)) + int(raw_part.get("bat2fours", 0))
                sixes = int(raw_part.get("bat1sixes", 0)) + int(raw_part.get("bat2sixes", 0))
...
def extract_players_from_scorecard(data: Dict[str, Any]) -> List[Player]:
    ...
    for raw_inn in raw_scorecards:
        ...
        for raw_bat in batsmen_list:
            player_id = int(raw_bat["id"])
            name = raw_bat.get("name") or raw_bat.get("nickname") or f"Player {player_id}"
        ...
        for raw_bowl in bowlers_list:
            player_id = int(raw_bowl["id"])
            name = raw_bowl.get("name") or raw_bowl.get("nickname") or f"Player {player_id}"
            players_dict[player_id] = Player(id=player_id, name=name)
